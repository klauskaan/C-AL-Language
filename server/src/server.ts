  ...
  try {
    ...
    semanticDebouncer.schedule(uri, version, (checkUri) => {
      ...
      try {
        ...
        const semanticDiagnostics = semanticAnalyzer.analyze(ast, symbolTable, uri, currentSettings, hasTableRegistry, fieldRegistry);
...
function parseDocument(document: TextDocument): ParsedDocument {
  ...
  symbolTable.buildFromAST(ast, workspaceIndex.getTableRegistry(), workspaceIndex.getFieldRegistry());
